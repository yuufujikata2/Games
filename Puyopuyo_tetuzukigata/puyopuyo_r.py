...
import numpy as np
import pickle
def allhyouji(haichi,nexnex,ojama,counttotal,counttotal2,screen,i):
...
def ojamahyouji(x,y,screen):
    ...
    for i in range(go2,go3):
      y[i]=1
    hyouji3(y,screen)
def ojamarakka(x,nexnex,y,counttotal,screen):
  global counttotal2
  while counttotal>2100:
    ...
    x.append([0,0,0,0,0,0])
    counttotal-=2100
    ojamahyouji(counttotal,y,screen)
    for j in range(13,18):
      for i in range(6):
        x[j][i]=5
    sokurakka(x,18,nexnex,y,counttotal,screen)
    x.pop(17)
    x.pop(16)
    x.pop(15)
    x.pop(14)
  ...
  for j in range(1,y):
    for i in range(6):
      if x[j][i]!=0 and x[j-1][i]==0:
        ...
        count+=1
  return count 
def rakka(haichi,y,nexnex,ojama,counttotal,screen):
  global counttotal2
  count2=0
  while True:
    count=0
    for j in range(1,y):
      for i in range(6):
        if haichi[j][i]!=0 and haichi[j-1][i]==0:
          ...
          haichi[j][i]=0
    allhyouji(haichi,nexnex,ojama,counttotal,counttotal2,screen,0)
    count2+=1
    if count==0:
      break
    time.sleep(0.8)
  return count2
def sokurakka(haichi,y,nexnex,ojama,counttotal,screen):
  global counttotal2
  count2=0
  while True:
    count=0
...
def rensa(x,y,z,i,screen):
  ...
  allhyouji(x,y,z,counttotal,counttotal2,screen,i)
  if j[0]!=0:
    time.sleep(0.8)
...
#              puyo2[1]=5
    ...
    allhyouji(haichi,nexnex,ojama,counttotal,counttotal2,screen,0)
    if counttotal>=70:
      ojamarakka(haichi,nexnex,ojama,counttotal,screen)
#    print(counttotal)
    counttotal=0
#    print (counttotal2)
    for event in pygame.event.get():
