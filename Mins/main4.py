...
#        pygame.draw.rect(self.screen,(0,0,250),Rect(0,1000,1000,95),10)
    ...
    def hyouji3(self,highscore):
        self.screen.fill((250,250,250))
        self.screen.blit(self.font3.render("High Score",False,(0,0,0)),(150,50))
        for i in range(5):
            if highscore.highscore[i] == None:
                self.screen.blit(self.font3.render(str(i + 1) + "   ...   ",False,(0,0,0)),(150,300 + 150 * i))

            else:
                self.screen.blit(self.font3.render(str(i + 1) + "   ...   " + str(highscore.highscore[i]) +"s",False,(0,0,0)),(150,300 + 150 * i))
        pygame.display.update()
...
class Highscore():
    def __init__(self):
        self.highscore = [None,None,None,None,None]

    def read(self):
        with open("highscore" , mode = "r") as fp:
            line = fp.readlines()
            for i in range(len(line)):
                if line[i].strip() == "None":
                    continue
                self.highscore[i] = int(line[i].strip())

    def write(self, field):
        high = field.time
        for j in range (5):
            if self.highscore[j] == None:
                self.highscore[j] = high
                break
            elif high <= self.highscore[j]:
                kari = self.highscore[j]
                self.highscore[j] = high
                high = kari
        with open ("highscore" , mode = "w") as fp:
            for i in range(5):
              fp.write(str(self.highscore[i]) + "\n")
